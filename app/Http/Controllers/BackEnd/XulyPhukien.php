...
class XulyPhukien extends Controller
{
    private $sanPham;
    private $laptop;
    private $phuKien;
    ...
    public function xulyphukien(Request $request)
    {
        ...
        if ($request->thaoTac == "xóa phụ kiện") { // *******************************************************************************************xoa phu kien
            ...
            if (!empty($thongTinChiTietPhieuNhap)) {
                return back()->with(
                    ...
                    'thongbao',
                    'Phụ kiện đã tồn tại trong phiếu nhập [PN' . $thongTinChiTietPhieuNhap[0]->id_purchase_order . '] nên không thể xóa'
                )->with(
                    'loaithongbao',
                    'danger'
                );
            ...
            if (!empty($thongTinChiTietPhieuXuat)) {
                return back()->with(
                    ...
                    'thongbao',
                    'Phụ kiện đã tồn tại trong phiếu xuất [PX' . $thongTinChiTietPhieuXuat[0]->id_invoice . '] nên không thể xóa'
                )->with(
                    'loaithongbao',
                    'danger'
                );
            ...
            if (!empty($thongTinSanPham)) {
                ...
                if ($thongTinSanPham->cat_products == 1 && !empty($thongTinSanPham->id_accessory )) {
                    $thongTinPhuKien = $this->phuKien->timPhuKienTheoMa($thongTinSanPham->id_accessory ); //tim phu kien
                    if (!empty($thongTinPhuKien)) {
                        $this->phuKien->xoaPhuKien($thongTinPhuKien->id_accessory ); //xoa phu kien tren database
                    }
                }
        ...
        if ($request->thaoTac == "sửa phụ kiện") { // *******************************************************************************************sua phu kien
            ...
            // ***********Xu ly sua phu kien
            if ($thongTinSanPham->cat_products == 1 && !empty($thongTinSanPham->id_accessory )) { // la phu kien
                $dataPhuKien = [
                    $thongTinSanPham->name_products,
                    $request->tenLoaiPhuKienSua
                ];
                $this->phuKien->suaPhuKien($dataPhuKien, $thongTinSanPham->id_accessory ); // sua thong tin phu kien tren database
            }
        ...
        if ($request->thaoTac == "thêm phụ kiện") { // *******************************************************************************************them phu kien
            ...
            $thongTinPhuKien = $this->phuKien->timPhuKienTheoTenSanPham($request->name_Products); //tim phu kien vua them
            ...
            $dataSanPham = [
                ...
                NULL, //ma lap top
                $thongTinPhuKien->id_accessory , //ma phu kien
                1 //loai san pham
                //ngaytao tu dong
            ];
